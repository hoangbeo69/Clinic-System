...
// Private helper vars
  ...
  var _ = {
    ...
    matchGrammar: function (text, strarr, grammar, index, startPos, oneshot,
        target) {
      for (var token in grammar) {
        ...
        for (var j = 0; j < patterns.length; ++j) {
          ...
          for (var i = index, pos = startPos; i < strarr.length;
            ...
            if (greedy && i != strarr.length - 1) {
              ...
              for (var len = strarr.length;
                  k < len && (p < to || (!strarr[k].type && !strarr[k
                  - 1].greedy)); ++k) {
                p += strarr[k].length;
            ...
            var wrapped = new Token(token,
                inside ? _.tokenize(match, inside) : match, alias, match,
                greedy);
  ...
  Token.stringify = function (o, language, parent) {
    ...
    return '<' + env.tag + ' class="' + env.classes.join(' ') + '"'
        + (attributes ? ' ' + attributes : '') + '>' + env.content + '</'
        + env.tag + '>';
...
//Get current script and highlight
  var script = document.currentScript || [].slice.call(
...
(function () {
  ...
  self.Prism.fileHighlight = function (container) {
    ...
    Array.prototype.slice.call(
        container.querySelectorAll('pre[data-src]')).forEach(function (pre) {
    ...
    if (Prism.plugins.toolbar) {
      Prism.plugins.toolbar.registerButton('download-file', function (env) {
        ...
        a.textContent = pre.getAttribute('data-download-link-label')
            || 'Download';
