...
(function (factory) {
    ...
    */
    ...
    _constructor: function () {
        ...
        if (ctx.aoColumns[i].checkboxes) {
          ...
          if (ctx.aoColumns[i].checkboxes.selectAll) {
            ...
            if (ctx.aoColumns[i].checkboxes.selectAllRender !== null) {
              ...
              } else if (typeof ctx.aoColumns[i].checkboxes.selectAllRender
                  === 'string') {
                selectAllHtml = ctx.aoColumns[i].checkboxes.selectAllRender;
              }
      ...
      if (hasCheckboxes) {
        ...
        if (hasCheckboxesSelectRow) {
          ...
          $table.on('user-select.dt.dtCheckboxes',
          ...
          $table.on('select.dt.dtCheckboxes deselect.dt.dtCheckboxes',
        ...
        $tableBody.on('click.dtCheckboxes', 'input.dt-checkboxes',
            function (e) {
              self.onClick(e, this);
            });
        ...
        $tableContainer.on('click.dtCheckboxes',
            'thead th.dt-checkboxes-select-all', function () {
              $('input[type="checkbox"]', this).not(':disabled').trigger(
                  'click');
            });
        ...
        $(document).on('click.dtCheckboxes',
        ...
        $table.on('stateSaveParams.dt.dtCheckboxes',
    ...
    onDataTablesInit: function () {
      ...
      if (!ctx.oFeatures.bServerSide) {
        ...
        $(dt.table().node()).on('xhr.dt.dtCheckboxes',
    ...
    updateCheckbox: function (cells, colIdx, isSelected) {
      ...
      if (cellNodes.length) {
        $('input.dt-checkboxes', cellNodes).not(':disabled').prop('checked',
            isSelected);

        // If selectCallback is a function
        if ($.isFunction(ctx.aoColumns[colIdx].checkboxes.selectCallback)) {
          ctx.aoColumns[colIdx].checkboxes.selectCallback(cellNodes,
              isSelected);
        }
      }
    },
      ...
      dt.cells('tr', self.s.columns, opts).every(function (rowIdx, colIdx) {
        ...
        if (
            ...
            && Object.prototype.hasOwnProperty.call(
                ctx.checkboxes.s.data[colIdx], cellData)
        ) {
    ...
    loadState: function () {
      ...
      if (ctx.oFeatures.bStateSave) {
        ...
        $.each(self.s.columns, function (index, colIdx) {
          ...
          if (state && state.checkboxes && state.checkboxes.hasOwnProperty(
              colIdx)) {
            // If checkbox state saving is enabled
            if (ctx.aoColumns[colIdx].checkboxes.stateSave) {
              // Load previous state
    ...
    updateSelectAll: function (colIdx) {
      ...
      if (ctx.aoColumns[colIdx].checkboxes
        ...
        var $checkboxesSelectAll = $(
            ...
            + '"] input[type="checkbox"]', $tableContainer);
        ...
        if (isChanged || isSelectedNow !== isSelected || isIndeterminateNow
            !== isIndeterminate) {
          ...
          if ($.isFunction(
            ...
            ctx.aoColumns[colIdx].checkboxes.selectAllCallback(
                $checkboxesSelectAll.closest('th').get(0), isSelected,
                isIndeterminate);
          }
        }
      }
    ...
    showInfoSelected: function () {
      ...
      if (colIdx !== null) {
        ...
        for (var cellData in ctx.checkboxes.s.data[colIdx]) {
          if (
              Object.prototype.hasOwnProperty.call(ctx.checkboxes.s.data,
                  colIdx)
              && Object.prototype.hasOwnProperty.call(
                  ctx.checkboxes.s.data[colIdx], cellData)
          ) {
    ...
    isCellSelectable: function (colIdx, cellData) {
      ...
      if (
          Object.prototype.hasOwnProperty.call(ctx.checkboxes.s.dataDisabled,
              colIdx)
          && Object.prototype.hasOwnProperty.call(
              ctx.checkboxes.s.dataDisabled[colIdx], cellData)
      ) {
        return false;
  ...
  */
  ...
  Api.registerPlural('columns().checkboxes.select()',
      'column().checkboxes.select()', function (state) {
  ...
  Api.registerPlural('cells().checkboxes.select()',
      'cell().checkboxes.select()', function (state) {
        if (typeof state === 'undefined') {
          state = true;
        }
  ...
  Api.registerPlural('cells().checkboxes.enable()',
      'cell().checkboxes.enable()', function (state) {
        if (typeof state === 'undefined') {
          state = true;
        }
