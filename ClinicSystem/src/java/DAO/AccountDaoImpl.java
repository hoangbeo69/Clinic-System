...
public class AccountDaoImpl extends AbstractDAO implements AccountDao {
    ...
    public Long save(Account user) {
        ...
        sql.append("(USERNAME,PASSWORD,FULLNAME,PHONENUMBER,EMAIL,DOB) Values");
        sql.append("(?,?,?,?,?,?)");
        return insert(sql.toString(), user.getUsername(), user.getPassword(), user.getFullName(),
                user.getPhoneNumber(), user.getEmail(), user.getDob());
    }

    @Override
    public Long update(Account user) {
        StringBuilder sql = new StringBuilder("UPDATE Account ");
        sql.append("SET USERNAME = ?,PASSWORD = ?,FULLNAME = ?,PHONENUMBER = ? ,EMAIL = ? ,DOB = ? WHERE ID = ?");
        boolean result = update(sql.toString(), user.getUsername(), user.getPassword(), user.getFullName(),
                user.getPhoneNumber(), user.getEmail(), user.getDob(), user.getId());
